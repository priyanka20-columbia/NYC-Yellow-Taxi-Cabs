...
##fixed plot5 plot9. fixed the preiction model using combined data and clusters
##delete a few unnecessary functions

#identify outlier

DataWithTimeStamp_YellowTaxi = read.csv("DataWithTimeStamp_YellowTaxi.csv")
#remove outliars identified
...
#standize the time
pickup_datetime = ymd_hms(data_final$tpep_pickup_datetime)
dropoff_datetime = ymd_hms(data_final$tpep_dropoff_datetime)
pickup_hour = hour(data_final$tpep_pickup_datetime)
dropoff_hour= hour(data_final$tpep_dropoff_datetime)
...
plot1
#majority of trip only has 1 passenger- car pool might be a good idea since it will lower the cost for indiviual- attract more people to take taxi and increase the total revenue
...
#find the top 5 pick zones instead (can use ggplot)
head(sort(table(data_final$PULocationID),decreasing=T)) 
#327 236 161 186 162 230

#top 5 drop off zone9(can use ggplot)
head(sort(table(data_final$DOLocationID),decreasing=T)) 
#236 237 161 230 162 170
...
#since cash tip will not be recorded by meter, payment 1 is credit card
...
#Way2 for above plot by adding other column 
data_final$pickup_hour <- as.POSIXlt(data_final$tpep_pickup_datetime)$hour
Pickup_hour_bifurcation <- data_final %>%
group_by(pickup_hour) %>%
   count()
Pickup_hour_bifurcation
plot6 <- ggplot(Pickup_hour_bifurcation, aes(x = pickup_hour, y = n, fill=pickup_hour)) +
...
#Way2
data_final$dropoff_hour <- as.POSIXlt(data_final$tpep_dropoff_datetime)$hour
Dropoff_hour_bifurcation <- data_final %>%
group_by(dropoff_hour) %>%
count()
Dropoff_hour_bifurcation
plot7 <- ggplot(Dropoff_hour_bifurcation, aes(x = dropoff_hour, y = n, fill=dropoff_hour)) +
theme_bw() + geom_bar(stat = "identity")


#plot 8 trip duartion , exploring
data_final$trip_duration_min <- 
floor( difftime(data_final$tpep_dropoff_datetime,data_final$tpep_pickup_datetime,units="mins") )

Trip_duration_bifurcation <- data_final %>%
group_by(trip_duration_min) %>%
count()
Trip_duration_bifurcation

plot8 <- ggplot(Trip_duration_bifurcation, aes(x = trip_duration_min, y = n, fill=trip_duration_min)) +
theme_bw() + geom_line()
plot8
tail(Trip_duration_bifurcation, 1040)
...
plot(cut(as.dendrogram(clust),h=6)$upper)
#cluster plot 
plot(clust)
...
clusplot(data_cluster,h_segments,color=T,shade=T,labels=4,lines=2,main='Hierarchical Cluster Plot')

within_ss = sapply(1:10,FUN = function(x){
  set.seed(1031)
  kmeans(x = data_cluster,centers = x,iter.max = 1000,nstart = 25)$tot.withinss})
#elbow
ggplot(data=data.frame(cluster = 1:10,within_ss),aes(x=cluster,y=within_ss))+geom_line(col='blue',size=1.5)+geom_point()+scale_x_continuous(breaks=seq(1,10,1))



#split data
data2 = cbind(clusterdata, h_segments)

set.seed(1031)
split = createDataPartition(y=data2$fare_amount,p = 0.7,list = F,groups = 100)
train = data2[split,]
test = data2[-split,]
#split data
set.seed(1031)
...
summary(lm_model)
pred1 = predict(lm_model,newdata=test)
mean(pred1)
mean(data_final$fare_amount)
